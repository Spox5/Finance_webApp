...
<?php
	...
	
	{
		...
		{
			$query = "SELECT e_c.name, SUM(e.amount) FROM expenses_category_assigned_to_users AS e_c, expenses AS e WHERE e.user_id = e_c.user_id AND e.expense_category_assigned_to_user = e_c.id AND e.user_id=$_SESSION[user_id] AND (YEAR (e.date_of_expense) = $current_year) GROUP BY e_c.name";
						...
						<div class="table1 col-sm-12 col-md-6">
							<table>
								...
								try
								{
									...
									{
										$result = $connect->query("SELECT i.user_id, i.amount, i.date_of_income, i.id, i.income_comment, i_c.name FROM incomes AS i, incomes_category_assigned_to_users AS i_c WHERE i.user_id = i_c.user_id AND i.income_category_assigned_to_user_id = i_c.id AND i.user_id=$_SESSION[user_id] AND (YEAR (i.date_of_income) = $current_year) ORDER BY date_of_income DESC" );
						...
						<div class="summary col-md-12">Podsumowanie bilansu:
							...
							require_once "connect.php";
								...
								try
								{
									...
									{
										$result_income = $connect->query("SELECT SUM(amount) as income_summary FROM incomes WHERE user_id=$_SESSION[user_id] AND (YEAR (incomes.date_of_income) = $current_year)");
										
										$result_expense = $connect->query("SELECT SUM(amount) as expense_summary FROM expenses WHERE user_id=$_SESSION[user_id] AND (YEAR (expenses.date_of_expense) = $current_year)");
